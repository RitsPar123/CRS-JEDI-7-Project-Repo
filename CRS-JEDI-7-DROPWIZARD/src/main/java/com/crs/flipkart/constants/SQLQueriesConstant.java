...
public class SQLQueriesConstant {
	...
	public static final String GET_STUDENT_ACOURSES_BY_ID = "select course.courseId,course.courseName,course.pId from course inner join registeredcourse on course.courseId = registeredcourse.courseId  where (SId = ? AND isRegistered = 1)";
	public static final String GET_STUDENT_SCOURSES_BY_ID = "select course.courseId,course.courseName,course.pId from course inner join registeredcourse on course.courseId = registeredcourse.courseId  where SId = ?";
	...
	public static final String VIEW_PROF_COURSES = "select CourseId, CourseName , SeatCount from course where PId = ? ";
	...
	public static final String VIEW_REGISTERED_STUDENTS = "Select user.id ,user.name , user.role , student.IsApproved , student.IsRegistered , student.IsReportApproved  from (user INNER JOIN (registeredcourse INNER JOIN student ON  registeredcourse.sid=student.sid  ) ON registeredcourse.sid = user.id ) where registeredcourse.courseid = ? and registeredcourse.IsRegistered = 1";
	...
	public static final String NOTIFICATION_STUDENT = "select * from notification where SId=?";
